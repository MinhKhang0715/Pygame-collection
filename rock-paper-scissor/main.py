...
status = Label(text="Let's play!", location=(
    SCREEN_WITDH // 2, SCREEN_HEIGHT // 2))
is_playing = False
      ...
      for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
          ...
          if restart_button.text_rect.collidepoint(mouse_pos):
            ...
            is_playing = False
          clicked_shape = [
              s for s in shape_group if s.rect.collidepoint(mouse_pos)]
          if not clicked_shape or not clicked_shape[0].is_clickable:
            continue
          else:
            shape_group.remove([chosen_shape, computer_chosen_shape])
            computer_choice = str(random.choice(choices))
            chosen_shape = Shape(clicked_shape[0].type, location=(
                200, SCREEN_HEIGHT // 2), is_clickable=False)
            computer_chosen_shape = Shape(computer_choice, location=(
                SCREEN_WITDH - 200, SCREEN_HEIGHT // 2), is_clickable=False)
